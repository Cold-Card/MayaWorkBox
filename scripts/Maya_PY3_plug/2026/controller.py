...
class CurveControllerEdit:
    ...
    def as_swap_curve(self, type, Name, Color, Num):
        ...
        for i in range(0, len(sel) - 1):
            ...
            for y in range(0, len(tempString)):
                ...
                attr_info = []
                if tempString3 and tempString:
                    if cmds.objExists(tempString3[0] + " " + tempString[y] + ".v"):
                        attr_info = cmds.getAttr(tempString3[0] + " " + tempString[y] + ".v", lock=True)
                # print(attr_info)
                if tempString3 and attr_info:
    ...
    def modify_vontroller_shape(self, deformation_type, x, y, z):
        ...
        for i in range(0, len(curve)):
            ...
            cmds.select(cl=1)
            for s in Shape:
                cmds.select((s + ".cv[0:]"),add=1)
            if deformation_type == "translate":
                cmds.move(x, y, z, r=1, os=1, wd=1)
    ...
    def old_create_fk(self, controller_group_num, suffix, if_remove_joint, extract_constraints):
        ...
        for i in range(0, len(joint)):
            cmds.rename(self.curve.create_curve((self.root_path +'\\'+ maya_version + '\curve_library'), 'A圆'), (joint[i] + suffix + '_C'))
            sel = cmds.ls(sl=1)
            curve.append(sel[0])
                        ...
                        if cmds.objExists(mirror_curve_name):
                            ...
                            for i in range(0,len(all_cv)):
                                ...
                                if cmds.objExists(mirror_curve_name+'.cv['+str(i)+']'):
                                    ...
                                    cmds.setAttr((mirror_curve_name+'.cv['+str(i)+']'), t[0]*-1, t[1]*-1, t[2]*-1)
    ...
    def create_controllers_system(self, controller_group_num, suffix, if_remove_joint, keep_curve_shape):
        ...
        if cmds.objExists('All_ControllerSystem_Grp') and cmds.objExists('BaseRoot_Grp'):
            ...
            # print(reduction_curve_dict_list)
            all_connect_node, Constraint = self.return_constraint(skin_joint + reduction_curve_dict_list + other_grp, [], [])
            # print(all_connect_node)
            # print(Constraint)
            # 创建蒙皮骨骼
                ...
                for g in range(0, controller_group_num):
                    if cmds.objExists((j + '_FKGrp' + str(g + 1))):
                        ...
                        if ls_grp:
                            cmds.parent(two_grp, ls_grp)
                    if ls_grp:
                        child = cmds.listRelatives(ls_grp, c=1)
                        if child:
                            # print(two_grp)
                            # print(child)
                            if not two_grp in child:
                                cmds.parent(two_grp,ls_grp)
                        else:
                            cmds.parent(two_grp, ls_grp)
                    # print(ls_grp)
                    ...
                    if child:
                        ...
                        for c in child:
                            ...
                            if c and our and cmds.objExists(c) and cmds.objExists(our):
                                child_now = cmds.listRelatives(our, c=1)
                                if child_now:
                                    if not c in child_now:
                                        cmds.parent(c, our)
                                else:
                                    # print(c)
                                    cmds.parent(c, our)
            if all_connect_node:
                self.return_constraint([], all_connect_node, Constraint)
            # print(all_connect_node, Constraint)
