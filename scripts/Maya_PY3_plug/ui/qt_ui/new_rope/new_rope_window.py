...
class Window(QtWidgets.QMainWindow):
    def __init__(self, parent=wrapInstance(int(Omui.MQtUtil.mainWindow()), QtWidgets.QWidget)):
        ...
        current_file = Path(os.path.abspath(inspect.getsourcefile(lambda: 0)))
        # 文件路径
        self.file_path = current_file.parent
        # 根路径
        ...
        # 样条库路径
        self.curve_library_path = os.path.join(self.library_path, 'curve_library')
    ...
    def create_rope(self):
        ...
        if cmds.objExists(controller_orientation_joint):
            ...
            curve = self.curve.create_curve(os.path.join(self.library_path, 'curve_library'), '四边方向箭')
    ...
    def create_independent_con(self):
        ...
        for i in range(len(self.joint_chain)):
            curve = self.curve.create_curve(os.path.join(self.library_path, 'curve_library'), '正方形')
    ...
    def create_base_ik_con(self, new_curve, all_position_out_joint):
        ...
        for i in range(len(self.controller_orientation_joint)):
            curve = self.curve.create_curve(self.curve_library_path, '十字星')
        ...
        for i in range(0,len(all_curve),3):
            ...
            curve = self.curve.create_curve(self.curve_library_path, '圆片')
    ...
    def drag(self, all_ik_top_grp):
        ...
        for i in range(len(all_curve_point)):
            ...
            cluster = cmds.cluster()
            curve = self.curve.create_curve(self.curve_library_path, '正方形')
