...
def fix_adv_spine():
    ...
    # add IKExtraSpine5_M ctrl
    end_ctl = ikSplines[-1]
    end_shape = end_ctl.getShape()
    end_parent = end_ctl.getParent()
    # Set Pivot
    Spine3_M = pm.PyNode('Spine3_M')
    spivot = Spine3_M.getScalePivot(space='world')
    end_parent.setScalePivot(spivot, space='world')
    end_parent.setRotatePivot(spivot, space='world')

    with createControl(end_shape, end_parent) as tmpCurve:
        adjustShapeStyle_ladder(tmpCurve)
        tmpCurve.ove.set(1)
        tmpCurve.ovc.set(17)
    ...
    connect_shape(end_parent, end_ctl)
...
def ZningAddFix():
    ...
    pm.select(cl=True)
    pm.inViewMessage(amg='Fix ADV Spine_Q: <hl>Fianl</hl>',pos='midCenterTop',fade=False,dk=True)
