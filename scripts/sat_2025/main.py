...
# Embedded file name: F:/Maya_Projects\sat\main.py
# Compiled at: 2016-11-26 20:42:45
import maya.cmds as cmds
import maya.mel as mel, pymel.core as pm
...
import os, imp, types, webbrowser, logging, inspect
...
class MainWindow(QtWidgets.QMainWindow, mainWindow.Ui_MainWindow):
    ...
    def connectSignals(self):
        ...
        self.sculpt_btn.clicked.connect(self.sculpt)
        ...
        def onPress():
            ...
            for mesh in cmds.ls(type='mesh'):
                ...
                if intersection:
                    ...
                    if not o.isIntermediate() and o.isVisible():
    ...
    def sculpt(self, *args):
        ...
        if self.nowType == False:
            self.nowType = True
        else:
            self.nowType = False
        self.editMode = self.nowType
        ...
        if self.editMode:
            ...
            self.sculpt_btn.setStyleSheet('')
        self.brush_btn.setEnabled(self.nowType)
        self.points_btn.setEnabled(self.nowType)
        if useShapesBrush:
            self.shapesBrush_btn.setEnabled(self.nowType)
        self.resetShape_btn.setEnabled(self.nowType)
        cmds.setAttr(self.curMesh + '.lodVisibility', not self.nowType)
        self.actionUse_Artisan_Tool.setEnabled(self.nowType)
        self.actionUse_ShapesBrush_plugin.setEnabled(self.nowType)
        self.actionUse_Components.setEnabled(self.nowType)
        self.actionReset_Shape_to_Default.setEnabled(self.nowType)
    ...
    def saveData(self):
        logger.debug('Start ' + inspect.stack()[0][3])
        # print(self.meshes, self.curLayer, self.editMode)
