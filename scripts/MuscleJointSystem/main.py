...
if qtVersion.startswith("4") or type(qtVersion) is not str:
    # Maya 2016 及以前（Qt4）
    ...
    # Maya 2024 / 2025 / 2026（Qt6 -> PySide6）
...
class MainWindow():
    ...
    def __init__(self):
        ...
        # 兼容 PySide6 枚举写法，PySide2/PySide 同样支持
        self.ui.setWindowFlags(QtCore.Qt.WindowType.Dialog)

        # edit widget behavior
        ...
        # ui_list
        self.ui.Mjs_list.setSelectionMode(SelectionMode.ExtendedSelection)
        self.ui.Mjs_list.itemSelectionChanged.connect(self.refreshSlotsFuntion)
        self.ui.Mjs_list_2.setSelectionMode(SelectionMode.ExtendedSelection)
    ...
    def getMayaCurveEditor(self, pull=0, push=0, save=[]):
        ...
        if pull == 1:
            ...
            cmds.optionVar(rm='pushLimit_mtj')
            for ii in range(len(save)):
                cmds.optionVar(stringValueAppend=['pushLimit_mtj', save[ii]])
            cmds.gradientControlNoAttr('MycurveEditor', e=True, optionVar='pushLimit_mtj')
        else:
    ...
    # ---------- 以下业务逻辑与原文件完全一致，只把 shiboken2.wrapInstance 换成了 wrapInstance ----------
                    ...
                    if control:
                        ...
                        for aa in control:
                            if cmds.objectType(aa) == 'joint':
                                joint.append(aa)
                        jointNum = len(joint)
                        ScaleNum = cmds.getAttr(
                            uu.split('_nurbs_')[0] + '_arcLine_muscleDriver_infoNode.arcLength') / 4
                        ratio = 0.0
    ...
    def restoreBindPose(self):
        ...
        if sel:
            for uu in sel:
                shape = cmds.listRelatives(uu, s=True)[0]
                if cmds.objectType(shape) == 'nurbsSurface' and '_Mjs' in uu \
                        and len(cmds.ls(uu + '*muscleDriver*')) != 0:
                    value = cmds.getAttr(
                        uu.split('_nurbs_Mjs')[0] + '_arcLine_muscleDriver_infoNode.arcLength')
                    cmds.setAttr(
            ...
            if cmds.objectType(sel[0]) == 'joint' and '_Mjs' in sel[0]:
                ...
                if num == 1:
                    ...
                    for ss in pullNum:
                        ...
                        cmds.optionVar(stringValueAppend=['pullLimit_mtj',
                                                          '{0},{1},{2}'.format(str(bb), str(aa), str(cc))])
                    saveVar = cmds.optionVar(q='pullLimit_mtj')
                    print(saveVar)
                    self.ui.Mjs_CurveEditLayout.addWidget(self.getMayaCurveEditor(pull=1, save=saveVar))
                elif num == 2:
                    pushNumLong = cmds.getAttr(pushNode + '.value', mi=True)
                    pushNum = []
                    for ss in pushNumLong:
                        pushNum.append(int(ss))
                    cmds.optionVar(rm='pushLimit_mtj')
                    for ss in pushNum:
                        ValueName = pushNode + '.value[{0}]'.format(str(ss))
                        aa = cmds.getAttr(ValueName + '.value_Position')
                        bb = cmds.getAttr(ValueName + '.value_FloatValue')
                        cc = cmds.getAttr(ValueName + '.value_Interp')
                        cmds.optionVar(stringValueAppend=['pushLimit_mtj',
                                                          '{0},{1},{2}'.format(str(bb), str(aa), str(cc))])
                    saveVar = cmds.optionVar(q='pushLimit_mtj')
                    print(saveVar)
                    self.ui.Mjs_CurveEditLayout.addWidget(self.getMayaCurveEditor(push=1, save=saveVar))
        else:
            self.ui.Mjs_CurveEditLayout.addWidget(self.getMayaCurveEditor())
