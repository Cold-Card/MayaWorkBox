...
def instantQPatch():
    cleanList = ('innerLoop','oldSelLoop','selToPatchFace')
    for c in cleanList:
        if mc.objExists(c):
            mc.delete(c)
    selEdge = mc.filterExpand(sm = 32)
    faceCase = mc.filterExpand(sm = 34)
    meshName = ''
    if faceCase:
        currentSel = mc.ls(sl=1,fl=1)
        meshName = mc.ls(hl=1)
        mc.sets(name='selToPatchFace', text='selToPatchFace')
        mc.ConvertSelectionToEdgePerimeter()
        mc.DetachComponent()
        mc.polySeparate(meshName)
        mc.delete(all=1, e=1, ch=1)
        seperateMeshs = mc.ls(sl=1,fl=1)
        selFaceA = mc.sets('selToPatchFace', query=True)
        badMesh = selFaceA[0].split('.')[0]
        seperateMeshs.remove(badMesh)
        meshToPatch = seperateMeshs
        if mc.objExists('selToPatchFace'):
            mc.delete('selToPatchFace')
        mc.select(badMesh)
        mc.ConvertSelectionToEdgePerimeter()
        selEdge = mc.filterExpand(sm = 32)
    ...
    if selEdge:
        ...
        if curves:
            ...
            mc.delete(curves)
            if faceCase:
                mc.select(meshToPatch, add=True)
                mc.polyUnite(ch=0, mergeUVSets=1, name=meshToPatch[0])
                newName = mc.ls(selection=True)
                mc.polyMergeVertex(distance= 0.001, am=True, ch=0)
                if len(meshName)>1:
                    transNode = mc.ls(meshName, transforms =1)
                    meshName =   transNode                 
                mc.delete(meshName)
                mc.rename(meshName)
                mc.polyNormal(normalMode=2, userNormalMode=0, ch=0)
                mc.SetToFaceNormals()
                mc.select(cl=1)
                mc.select(meshName[0], replace=True)
                cmd  = 'doMenuComponentSelectionExt("' + meshName[0] + '", "facet", 0);'  
                mel.eval(cmd)
            else:
                mc.select(selGeo, add=True)
...
def instantQPatchUI():
    ...
    if mc.window("iconWin", exists=True):
        mc.deleteUI("iconWin")
    win = mc.window("iconWin", title="QPatch V1.1", mxb = False, mnb = False, s = 1 )
