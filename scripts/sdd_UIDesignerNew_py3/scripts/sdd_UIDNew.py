...
# dd=GenerateOptionEditor(True)
# dd.show()

class AlphaEditor():
    winTitle='sdd_AlphaEditor'
    def __init__(self,isModal=False):
        self.isModal=isModal
        self.outData=None
        self.inputData=None
...
class ToolImageEditor():
    winTitle='sdd_ToolImageEditor'
    def __init__(self,isModal=True):
        self.isModal=isModal
        self.imageSelect=IconChooser()
        self.outData=None
        self.editorType='tool'#'ToolIamge'#'Tool'
...
class ListEditor():
    ...
    def listEditorPrompt(self,form=None):
        ...
        for i in range(self.itemNum):
            ...
            cmds.button(l='Enalbe',w=60,c=functools.partial(self.enableChange,vCtrl))
...
# lve=ListEditor(False)
# print lve.show()

class FormEditor():
    winTitle='sdd_attachForm'
    def __init__(self,isModal=True):
        self.isModal=isModal
    ...
    def show(self,upDataFunction,argc,childList,inputDataList):
        ...
        if self.isModal:
            ...
            cmds.showWindow(self.winTitle)
# afe=FormEditor()
# af=[['sevSriptSTF', 'right', 2], ['sevSriptSTF', 'left', 2], ['sevOKB', 'left', 2], ['sevCancelB', 'bottom', 2], ['sevCancelB', 'right', 2], ['sevOKB', 'bottom', 2], ['sevOptionRL', 'top', 2], ['sevOptionRL', 'left', 2], ['sevOptionRL', 'right', 2]]
# ap=[['sevOKB', 'right', 0, 50]]
# ac=[['sevCancelB', 'left', 2, 'sevOKB'], ['sevSriptSTF', 'bottom', 2, 'sevOKB'], ['sevSriptSTF', 'top', 2, 'sevOptionRL']]
# print afe.show(['sevCancelB','sevOKB','sevSriptSTF','sevOptionRL'],[af,ap,ac])

class ScriptEditor():
    winTitle='sdd_ScriptEditorView'
...
class PropertyDelegate():
    ...
    def listEditor(self,*argc):
        ...
        if(className in ['rowLayout','shelfTabLayout','tabLayout']):
            if(longName in ['columnWidth','columnAlign','tabLabelIndex']):
                itemNum=len(self.UIDesigner.sdd_getSelectItemChild())
    ...
    def formEditor(self,*argc):
        ...
        childList=self.UIDesigner.sdd_getSelectItemChild()
    ...
    def refreshWindow(self):
        self.UIDesigner.sdd_generateCode()
        self.UIDesigner.sdd_showItemProperty()
...
class UIDesigner():
    ...
    def setupUI(self):
        ...
        cmds.window(self.winTitle,h=700,w=1000,mb=1)
        ...
        cmds.iconTextCheckBox('uidWinCacheITB',p='uidToolBarRL',v=1,i='deleteCacheHistory.png',ann='Delete Window Cache',w=23,h=23,cc=self.sdd_generateCode)
        cmds.iconTextCheckBox('uidDebugModeITB',p='uidToolBarRL',v=0,i='rvIPRStop.png',ann='Debug Code',w=23,h=23,cc=self.sdd_generateCode)
        ...
        cmds.iconTextButton(p='uidToolBarRL',i='ShortOpenBar.png')
        cmds.iconTextButton(p='uidToolBarRL',i='redrawPaintEffects.png',w=23,h=23,c=self.sdd_refreshWindow)
        ...
        cmds.treeView('uidHierarchyTV',p='uidHierarchyFL',elc=self.treeItemRenameLabel,scc=self.sdd_treeSelectChange,ecc=self.treeViewExpendChange,dad=self.treeItemHierarchyChange)
        #dad=sdd_dragAndDrop,elc=sdd_treeViewRenameLabel,scc='sdd_treeViewSelectChange()',ecc=sdd_treeViewExpendChange
        cmds.popupMenu()
    ...
    def undo(self,*argc):
        ...
        self.uidGLData=self.sdd_eval(dateStr)

        self.updateUndoRedoBtn()
        self.sdd_refreshWindow()
        self.sdd_treeSelectChange()
    ...
    def setFilePath(self,path):
        ...
        cmds.window(self.winTitle,e=1,t=path)
    ...
    def saveFileAs(self,*argc):
        ...
        try:
            ...
            self.dd_warning('Save %s Error!'%self.currentFilePath)
        finally:
            try:
                fileHandle.close()
            except:
    ...
    def getSameLevelList(self,selI):
        pLayout=self.sdd_getParentId(selI)
    ...
    def pasteControl(self,*argc):
        ...
        for ctrlId in pasteList:
            ...
            if not(pType in parentList):
                self.sdd_warning('Cann\'t parent %s to %s'%(ctrlId,pLayout))
                return
    ...
    def deleteControl(self,isQuery=True):
        ...
        winId=ctrlSort[0]
        #select parent
        pLayout=self.sdd_getParentId(selI[0])
        if(pLayout==None):
            pLayout=self.uidGLData['ctrlSort'][0]
        self.sdd_setSelectTreeItem(pLayout)
        for i in selI:
            if(i==winId):continue
            chList=cmds.treeView('uidHierarchyTV',q=1,ch=i)
            for child in chList:
    ...
    def sdd_loadUIWidget(self):
        ...
        for idx,mStr in enumerate(self.modulesList):
            ...
            for i in rootModule.__all__:
                itb=cmds.iconTextButton('uid%sITB'%i,st='iconAndTextHorizontal', l=i,c=functools.partial(self.sdd_addWidget,mStr,i))
    ...
    def replaceLayout(self,*argc):
        ...
        pItem=self.sdd_getParentId(ctrlId)
        newCtrlId,newBaseNode,selected=self.createBaseNode(moduleName,className)
        ...
        self.sdd_setParentAttr(newCtrlId,pItem)

        self.sdd_refreshWindow()
        self.sdd_setSelectTreeItem(newCtrlId)
    ...
    def sdd_initTreeView(self):
        self.uidGLData={'ctrlSort':[],'curSelTreeItem':None,'sourceType':'python','suffix':'_ui'}
        self.sdd_addWidget('Windows','window')

    def sdd_addWidget(self,moduleName,className):
        ...
        self.sdd_setParentAttr(ctrlId,pItem)
        self.sdd_refreshWindow()
        if(selected):
            self.sdd_setSelectTreeItem(ctrlId)
        else:
            self.sdd_setSelectTreeItem(pItem[0])
        self.addUndoAndClearRedo('sdd_addWidget')
    ...
    def createBaseNode(self,moduleName,className):
        ...
        ctrlId=self.sdd_getNonRepeatName(procName,className)
        return ctrlId,baseNode,ctrlModule.selected


    def sdd_setSelectTreeItem(self,item):
        cmds.treeView('uidHierarchyTV',e=1,cs=1)
    ...
    def treeItemHierarchyChange(self,*argc):
        ...
        for chName in chList:
            ...
            if not(pType in parentList):
                self.sdd_warning('Cann\'t parent %s  to %s'%(chName,newPname))
                self.sdd_refreshWindow()
                return
        for chName in chList:
            self.sdd_setParentAttr(chName,newPname)
    ...
    def sdd_treeSelectChange(self):
        ...
        self.sdd_updateCButtonState()
        self.updateMoveLevelButton()
        self.updateReplaceButton()
        self.sdd_showItemProperty()
    ...
    def treeItemRenameLabel(self,*argc):
        ...
        exists=self.sdd_uiExists(procName,newName)
        if(exists):
            self.sdd_warning('The name is repeated!')
            return ''
        ...
        self.sdd_refreshWindow()
        self.sdd_setSelectTreeItem(newName)
    ...
    def renameChangeData(self,oldName,newName):
        ...
        for ctrlId in ctrlSort:
            pName=self.sdd_getParentId(ctrlId)
            if(pName==oldName):
                self.sdd_setParentAttr(ctrlId,newName)

        idx=ctrlSort.index(oldName)
        ctrlSort[idx]=newName


    def sdd_refreshSelectTreeItem(self):
    ...
    def sdd_showItemProperty(self):
        ...
        self.sdd_getDefultData(ctrlIdList[0])
        pd.setCurrentCtrlBaseNode(baseNodeList)

    def sdd_updateCButtonState(self):
        selItem=cmds.treeView('uidHierarchyTV',q=1,si=1)
        if(selItem==None):return
        ctrlId=selItem[0]
        baseNode=self.uidGLData[ctrlId]
    ...
    def checkNeedParentFlag(self,ctrlId):
        ...
        if(className=='menuItem'):
            pCtrl=self.sdd_getRealParent(ctrlId)
    ...
    def checkNeedControlID(self,ctrlId):
        ...
        attrList=self.sdd_getAttrList(ctrlId)
    ...
    def renameFormData(self,oldName,newName):
        pLayout=self.sdd_getRealParent(oldName)
        if(pLayout==None):
            return
        if(self.uidGLData[pLayout]['className']!='formLayout'):
            return
        attrList=self.sdd_getAttrList(pLayout)
        for attr in attrList:
    ...
    def checkFormDataInfo(self,ctrlId):
        pLayout=self.sdd_getRealParent(ctrlId)
    ...
    def sdd_generateCode(self,*argc):
        if(self.uidGLData==None):
            return
    ...
    def generatePyCode(self):
        ...
        for ctrlId in ctrlSort:
            ...
            #parent flag
            pFlagStr=self.sdd_getPyOutputValue(ctrlId,attrList[0])
            #other flag
            for attr in attrList[1:]:
                flagName=self.getFlagName(attr)
                if(attr['active'] and (attr['value']!=None)):
                    if(attr['mode']=='c' or attr['mode']=='s'):
                        flagStr+=self.sdd_getPyOutputValue(ctrlId,attr)
                    elif(attr['mode']=='e'):
                        editFlagStr+=self.sdd_getPyOutputValue(ctrlId,attr)
                    elif(attr['mode']=='a'):
                        if(attr['type']=='dock'):
                            winId=ctrlSort[0]
    ...
    def sdd_getPyOutputValue(self,ctrlId,attr):
        flagName=self.getFlagName(attr)
        if(attr['longName']=='parent'):
            if(self.checkNeedParentFlag(ctrlId)):
                pName=self.sdd_getRealParent(ctrlId,attr)
    ...
    def generateMelCode(self):
        ...
        for ctrlId in ctrlSort:
            ...
            #parent flag
            pFlagStr=self.sdd_getMelOutputValue(ctrlId,attrList[0])

            #other flag
            for attr in attrList[1:]:
                if(attr['active'] and (attr['value']!=None)):
                    if(attr['mode']=='c' or attr['mode']=='s'):
                        flagStr+=self.sdd_getMelOutputValue(ctrlId,attr)
                    elif(attr['mode']=='e'):
                        editFlagStr+=self.sdd_getMelOutputValue(ctrlId,attr)
                    elif(attr['mode']=='a'):
                        if(attr['type']=='dock'):
    ...
    def sdd_getMelOutputValue(self,ctrlId,attr):
        flagName=self.getFlagName(attr)
        if(attr['longName']=='parent'):
            if(self.checkNeedParentFlag(ctrlId)):
                pName=self.sdd_getRealParent(ctrlId,attr)
    ...
    def checkValidity(self):
        ...
        for ctrlId in ctrlSort:
            ...
            chList=self.sdd_getSelectItemChild(ctrlId)
            if(className=='window'):
                for i in attrList:
    ...
    def sdd_getDefultData(self,ctrlId):
        self.sdd_checkPreviewWindow()
        self.checkValidity()
    ...
    def sdd_checkPreviewWindow(self):
        winId=self.uidGLData['ctrlSort'][0]
        winId=self.getCtrlId(winId)
        if(not cmds.window(winId,q=1,ex=1)):
            self.sdd_generateCode()

    def sdd_deleteWindowPref(self,*argc):
        winId=self.uidGLData['ctrlSort'][0]
        winId=self.getCtrlId(winId)
        tlc=None
        if(cmds.window(winId,q=1,ex=1)):
    ...
    def sdd_getRealParent(self,ctrlId,attr=None):
        ctrlSort=self.uidGLData['ctrlSort']
        if(attr==None):
        ...
        if(attr.get('virtual',False)):
            ...
            realparent=self.sdd_getParentId(virtualParent)
            return realparent
        else:
            return attr['value']

    def sdd_getSelectItemChild(self,selCtrl=None):
        if(selCtrl==None):
    ...
    def getRealChildList(self,ctrlId):
        ...
        childList=[]
        for i in ctrlSort:
            pName=self.sdd_getRealParent(i)
            if(pName==ctrlId):
                childList.append(i)
        return childList


    def sdd_getNonRepeatName(self,procName,className,i=1):
        suf=self.getSufName()
        while self.sdd_uiExists(procName,className+str(i)):i+=1
        return className+str(i)

    def sdd_uiExists(self,procName,newName):
        if(procName=='window'):
    ...
    def setSufName(self,suf):
        ...
        if(suf!=''):
            if(not self.nameRegex.match(suf)):
                self.sdd_warning('The Suffix name is not in conformity with the rules!')
                return
        self.uidGLData['suffix']=suf
        self.sdd_refreshWindow()
    ...
    def sdd_refreshWindow(self,*argc):
        if(self.uidGLData==None):
            return
        ctrlSort=self.uidGLData['ctrlSort']
        cmds.treeView('uidHierarchyTV',e=1,ra=1)
        for i in ctrlSort:
            baseNode=self.uidGLData[i]
            cmds.treeView('uidHierarchyTV',e=1,ai=[i,self.sdd_getParentId(i)],ei=[i,baseNode['expend']])
            cmds.treeView('uidHierarchyTV',e=1,dls=[i,self.getSufName()])

        self.uidGLData['ctrlSort']=cmds.treeView('uidHierarchyTV',q=1,ch=1)

        self.sdd_generateCode()
        self.sdd_refreshSelectTreeItem()
        

    def sdd_setParentAttr(self,ctrlId,pItem):
        if(pItem==None):
            return
        ...
        attrList[0]['active']=1
    def sdd_getParentId(self,ctrlId):
        attrList=self.sdd_getAttrList(ctrlId)
        return attrList[0]['value']
    def sdd_getAttrList(self,ctrlId):
        baseNode=self.uidGLData[ctrlId]
        return baseNode['attrList']
    def sdd_eval(self,sCmd):
        import sys
