...
def createConditionRig(nubs_surface, global_scale_attr):
    ...
    cnd_node.rename(nubs_surface+'_condition')
...
def createStretchAbleRig(nubs_surface_shape, dup_curve_shape, cnd_node, index_value, out_node, stretch_attr):
    ...
    divi_node.rename(out_node+'_multiplyDivide')
    ...
    blend_node.rename(out_node+'_blendColors')
    ...
    mp_node.rename(out_node+'_motionPath')
    ...
    cpos_node.rename(out_node+'_closestPointOnSurface')
    ...
    posi_node.rename(out_node+'_pointOnSurfaceInfo')
    ...
    aim_node.rename(out_node+'_aimConstraint')
    posi_node.tangentU >> aim_node.target[0].targetTranslate
    posi_node.normal >> aim_node.worldUpVector
...
def main(self, obj_list, subJntCount, 
    ...
    if self.no_ctrl.getValue() and self.selected_type.getSelect() == 2:
        nubSurface = obj_list[0]
        maxParU = nubSurface.getShape().maxValueU.get()
    else:
        curves = []
        for obj in obj_list:
    ...
    sgrps = []
    mode = 1
    top_grp = createNode('transform','{}_rig_all_grp'.format(base_name))
    rigNoTranGrp = createNode('transform','{}_rigNoTranslate_grp'.format(base_name))
    ...
    if mode == 0:
        for i in range(subJntCount+1):
            ...
            sjnt = pm.createNode('joint', name='{}_skin_{}_jnt'.format(base_name, i+1))
            pm.parent(sjnt, sgrp)
            ftrans.t >> sgrp.t
            ftrans.r >> sgrp.r
            sgrps.append(sgrp)
        ...
        if self.no_ctrl.getValue():
            fk_end_ctl = nubSurface
        else:
            fk_end_ctl = fk_grp_list.get('ctl')[-1][0]
        ...
        pm.group(aim_node_list, n='{}_aimConstraint_grp'.format(base_name))
...
class Win(object):
    ...
    def mkMainLayout(self):
        colu = pm.columnLayout(adj=True,co=['both',10])
        pm.separator(h=5, style='none',parent=colu)
        self.mode_rbg = pm.radioButtonGrp(cw=[1,85],select=1, label='Hierarchy', labelArray2=['Selected', 'Below'], numberOfRadioButtons=2,parent=colu)
        pm.separator(h=5, style='none',parent=colu)
        self.create_type_rbg = pm.radioButtonGrp(cw=[1,85],select=1, label='Create type', labelArray2=['Sub control', 'Normal control'], numberOfRadioButtons=2,parent=colu)
        pm.separator(h=5, style='none',parent=colu)
        rowC = pm.rowColumnLayout(nc=2)
        self.no_ctrl = pm.checkBox(l=u'No Ctrl',v=False, parent=rowC, cc=lambda *args: self.onNoCtrl())
        self.selected_type = pm.radioButtonGrp(en=False, cw=[1,85],select=1, label='Selected type', labelArray2=['Joint', 'Surfaces'], numberOfRadioButtons=2, parent=rowC)
        pm.separator(h=5, style='none',parent=colu)
        self.skinJntCount = pm.floatSliderGrp(cw=[1,85], label='Sub joint count', field=True, minValue=0.5, maxValue=4, fieldMinValue=0.5, fieldMaxValue=300, value=2,parent=colu)
        pm.separator(h=5, style='none',parent=colu)
        pm.button(l='Go', c=lambda *args: self.create(),parent=colu)
        pm.separator(h=10, style='none',parent=colu)

    def onNoCtrl(self):
        if self.no_ctrl.getValue():
            self.selected_type.setEnable(True)
        else:
            self.selected_type.setEnable(False)
    ...
    def create(self):
        ...
        no_ctrl = self.no_ctrl.getValue()
        selected_type = self.selected_type.getSelect()
        ...
        for objs in objlists:
            objCount = len(objs)
            if no_ctrl and selected_type == 2:
                subJntCount = count-1
            elif objCount > 1:
                subJntCount = int((objCount - 1) * count)
            if objCount > 1 or (no_ctrl and selected_type == 2):
                main(self, objs, subJntCount, grp_type_list=grp_type_list,
