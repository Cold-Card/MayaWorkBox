...
oa_ribbonizer.py 
version 2.0.1
copyright (C) 2021 Orkhan Ashrafov

given a nurbs surface creates a ribbon. if nothing is selected the button creates a default plane ribbon.

---------------------------------------------------------------------------------------------------------------------------------------------

install instructions: place this file in the default "scripts" folder. go to maya and run these two lines from a python tab: 
 
import oa_ribbonizer
oa_ribbonizer.UI()
...
no difference with the results unless the surface is deformed after creation (i.e: cvs have been moved). 
...
WIRE - deform the surface using wireDeformer instead of skinCluster. skinCluster is used if there's only 1 control
...
Version history:
2.0.1 - changed a few things for Python 3 support
        fixed bind joint rotation issue when using wire mode
...
def ribbonize(surf_tr, equal=1, num_of_ctrls=5, num_of_jnts=29, prefix="", constrain=1, add_fk=0, wire=0):
    ...
    if wire == True and num_of_ctrls > 1:
        ...
        wire_grp = mc.group(wire_crv, n=wire + "_grp")
        wire_constr_grp = mc.group(surf_tr, wire_crv + "BaseWire", n=wire + "_constr_grp")

        mc.parentConstraint(main_ctrl, wire_constr_grp, mo=True)
        mc.scaleConstraint(main_ctrl, wire_constr_grp)

        mc.setAttr(wire_crv + ".v", False)
        mc.setAttr(wire_grp + ".v", False)
        mc.setAttr(surf_tr + ".v", True)

        mc.parent(wire_constr_grp, wire_grp)
        mc.parent(wire_grp, rig_grp)
        lock_hide([wire_grp, wire_constr_grp], attrs[:9])
...
class UI(object):
    ...
    def __init__(self):
        ...
        mainWindow = mc.window(windowName, wh=(windowWidth, windowHeight), t="Ribbonizer 2", s=False, mnb=False, mxb=False)
    ...
    def launch(self, *args):
        ...
        if not sel:
            surf_tr = mc.nurbsPlane(ax=(0, 1, 0), w=1, lr=24, d=3, u=1, v=10, ch=False)[0]
            sys.stdout.write("we made a default ribbon for you. but you can also select any nurbs surface and ribbonize that.")
        else:
            surf_tr = sel
