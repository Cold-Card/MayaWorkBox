...
importlib.reload(dslDo)
importlib.reload(dslRs)
...
class SculptInbetweenEditor():
    ...
    def ui(self):
        ...
        if cmds.window('dsl_Sculpt_Inbetween_Editor', title=self.winName, ex=True):
            ...
            cmds.deleteUI('dsl_Sculpt_Inbetween_Editor')
            self.ui()
            ...
            try:
                ...
                print('listCorrective---', listCorrective)
            except:
                listCorrective = ['None']
                pass
            ...
            buttonMirrorData = cmds.button(' Mirror Delta ',
                                           ...
                                           # bgc=[colorBgc, colorBgc, colorBgc],
                                           parent='fieldApplyTo',
                                           c=partial(self.applyMirrorDelta, flipModel=False)
                                           )
            buttonFlipData = cmds.button(' Flip Delta ',
                                         ...
                                         c=partial(self.applyFlipDelta, flipModel=True)
                                         )
            buttonCopyData = cmds.button(' Copy Delta ',
                                         ...
                                         enable=False,
                                         # bgc=[0.68, 0.88, 0.68],
                                         c=partial(self.applyCopyDelta, copyMode=True),
                                         )
    ...
    def popupMenuCr(self, parentTo=None, arg=None):
        ...
        self.popOptAutoMirror = cmds.menuItem(label='Auto Mirror',
                                              parent=self.popUpMenuGadget,
                                              c=partial(self.singelMirror, autoMirror=True))
        self.popOptAutoMirror = cmds.menuItem(label='All Auto Mirror',
                                              parent=self.popUpMenuGadget,
                                              c=partial(self.allMirror, autoMirror=True))
    ...
    def listInbetween(self, inbetweenList=None,
        ...
        for idx, correctiveItem in enumerate(inbetweenList):
            ...
            try:
                ...
                if list(dicData.values())[0][0] == None:
    ...
    def checkIfData(self, blendShapeNode=None, corrective=None):
        ...
        print('cmds.getAttr(iTg + iTgGr + iTi + iPt)---', cmds.getAttr(iTg + iTgGr + iTi + iPt))
    ...
    def listCorrective(self, corrective=None, arg=None):
        ...
        if textFilter != None:
           ...
           '''
        print('textFilteredList---', textFilteredList)
        checkFilteredList = []
        ...
        print('checkFilteredList---', checkFilteredList)
        # intersectList = list(set(textFilteredList)&set(checkFilteredList))
        intersectList = []
        for i in textFilteredList:
            if i in checkFilteredList:
                intersectList.append(i)
        print('intersectList---', intersectList)
        if intersectList:
        ...
        # listFilteredCr = list(set(listFilteredCr))
        print('listFilteredCr---', listFilteredCr)
        cmds.textScrollList(self.scrollCrList, e=True, ra=True)
        cmds.textScrollList(self.scrollCrList,
                            e=True,
                            a=listFilteredCr,
    ...
    def listBlendShapeNode(self, refreshCrInb=False, selectedBlendShape=None, arg=None):
        ...
        print('selectedBlendShape-----', selectedBlendShape)
    ...
    def mirrorStringSide(self, selectedCr, arg=None):
        infoSide = []
        # Left & Right
        if 'L_' in selectedCr[:2]:
            selectedCr = selectedCr[1:]
            infoSide.append('R' + selectedCr)
            infoSide.append('YZ')
            return infoSide
        elif 'R_' in selectedCr[:2]:
            selectedCr = selectedCr[1:]
            infoSide.append('L' + selectedCr)
            infoSide.append('YZ')
            return infoSide
        else:
            om.MGlobal.displayError('The selected corrective must end with "L_" or "R_" to proceed with AutoMirror')

    def singelMirror(self, argv, autoMirror=None):

        fromCr = cmds.textFieldButtonGrp('fromCr', q=True, text=True)
        toCr = cmds.textFieldButtonGrp('toCr', q=True, text=True)
        self.applyMirrorOptionData(fromCr=fromCr, toCr=toCr, autoMirror=autoMirror)

    def allMirror(self, argv, autoMirror=None):

        items = cmds.textScrollList(self.scrollCrList, q=True, ai=True)
        for i in items:
            if 'L_' in i:
                toCr = cmds.textFieldButtonGrp('toCr', q=True, text=True)
                self.applyMirrorOptionData(fromCr=i, toCr=toCr, autoMirror=autoMirror)

    def applyMirrorDelta(self, argv, flipModel=False):
        fromCr = cmds.textFieldButtonGrp('fromCr', q=True, text=True)
        toCr = cmds.textFieldButtonGrp('toCr', q=True, text=True)
        self.applyMirrorOptionData(fromCr=fromCr, toCr=toCr, flipModel=flipModel)

    def applyFlipDelta(self, argv, flipModel=False):
        fromCr = cmds.textFieldButtonGrp('fromCr', q=True, text=True)
        toCr = cmds.textFieldButtonGrp('toCr', q=True, text=True)
        self.applyMirrorOptionData(fromCr=fromCr, toCr=toCr, flipModel=flipModel)

    def applyCopyDelta(self, argv, copyMode=False):
        fromCr = cmds.textFieldButtonGrp('fromCr', q=True, text=True)
        toCr = cmds.textFieldButtonGrp('toCr', q=True, text=True)
        self.applyMirrorOptionData(fromCr=fromCr, toCr=toCr, copyMode=copyMode)
        ...
        if sourceCr[0] != '' and destinationCr[0] != '' and sourceCr[0] != destinationCr[0]:
            ...
            for i in fromCrItems:
                ...
                self.newInbetween(blendShapeNode,
                                  destinationCr[0],
                                  newcorrectiveItem=i,
                                  pointArray=list(dataFromCr.values())[0][0],
                                  componentList=list(dataFromCr.values())[0][1])
    ...
    def duplicateCorrective(self, mirroed=False, arg=None):
        ...
        if (newCorrectiveName != selectedCr and newCorrectiveName != None
            ...
            for item in listCorrectiveItem:
                ...
                pointArray = list(dataInfo.values())[0][0]
                componentList = list(dataInfo.values())[0][1]
        ...
        if blendShapeNode != str(None):
            ...
            if checkCrExistence:
                ...
                if newCorrectiveName != None:
                    ...
                    # Get All Grp
                    groupsUnsorted = []
                    dicValues = list(self.getCrGrpItem().values())
    ...
    def getCrGrpItem(self, blendShapeNode=None,
        ...
        '''
        dicCrGrpItem = {}
        for crName in list(dicCrGrp.keys()):
    ...
    def restoreCorrective(self, blendShapeNode=None,
        ...
        delta = list(dataInfo.values())[0][0]
        vtxUnflatten = list(dataInfo.values())[0][1]
    ...
    def pasteDataCr(self, correctiveItem=None, arg=None):
        ...
        if crData != None:
            pArray = list(crData.values())[0][0]
            cList = list(crData.values())[0][1]
            self.newInbetween(blendShapeNode,
                              correctiveName,
    ...
    def resetSculpt(self, geo, copyMode=True):
        ...
        if copyMode:
            ...
            if listSculptPnts != None:
                for p in range(len(listSculptPnts)):
            ...
            if listDef != None:
                for d in listDef:
                    if cmds.objectType(d) == 'tweak':
                        ...
                        if listV != None:
                            for v in range(len(listV)):
                                # print vlistDir + '[%s]' %listV[v]
                                cmds.removeMultiInstance(vlistDir + '[%s]' % listV[v], b=True)
                            # Restore original Tweak connection
                            if cmds.isConnected(tweakNode + '.vlist[0].vertex[0]',
                                                shapeSculptGeo[0] + '.tweakLocation') != True:
                                cmds.connectAttr(tweakNode + '.vlist[0].vertex[0]',
                                                 shapeSculptGeo[0] + '.tweakLocation')
    ...
    def seeDelta(self, correctiveItem=None, selectAtTheEnd=True, arg=None):
        ...
        try:
            list(crData.values())[0][1].pop(0)
            # print '--------> ', blendShapeNode
            # print '--------> ', correctiveName
            # print '--------> ', geoName
            # print '--------> ', crData.values()[0][1]
            geoVtxList = []
            for vts in list(crData.values())[0][1]:
            ...
            return list(crData.values())[0][0][0]
        except:
