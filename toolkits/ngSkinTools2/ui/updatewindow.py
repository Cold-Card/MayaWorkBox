...
def show(parent, silent_mode):
    ...
    def body():
        ...
        header = QtWidgets.QLabel("<strong>正在检查更新...</strong>")
        result1 = QtWidgets.QLabel("当前版本：<strong>2.0.0</strong>")
        result2 = QtWidgets.QLabel("可用更新：2.0.1")
        download = QtWidgets.QPushButton("下载 ngSkinTools v2.0.1")
        ...
        def success_handler(info):
            ...
            header.setText("<strong>{0}</strong>".format("发现可用更新！" if info.update_available else "ngSkinTools 已是最新版本。"))
            result1.setVisible(True)
            result1.setText("当前版本：<strong>{0}</strong>".format(version.pluginVersion()))
            if info.update_available:
                result2.setVisible(True)
                result2.setText(
                    "可用更新：<strong>{0}</strong>，发布日期：{1}".format(info.latest_version, info.update_date.strftime("%Y-%m-%d"))
                )
                download.setVisible(True)
                download.setText("下载 ngSkinTools v" + info.latest_version)
    ...
    window.setWindowTitle("ngSkinTools2 版本更新")
    layout = QtWidgets.QVBoxLayout()
