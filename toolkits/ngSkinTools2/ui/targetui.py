...
from ngSkinTools2.ui.influence_tree_state import SEARCH_PLACEHOLDER, SEARCH_TOOLTIP
...
def build_layers_ui(parent, actions, session):
    ...
    def build_infl_filter():
        ...
        filter.lineEdit().setPlaceholderText(SEARCH_PLACEHOLDER)
        filter.lineEdit().setToolTip(SEARCH_TOOLTIP)
        filter.lineEdit().setWhatsThis(SEARCH_TOOLTIP)
        debounce = QtCore.QTimer(filter)
        ...
        clear.setIcon(img)
        clear.setToolTip("清空搜索并还原搜索前的骨骼树状态")
...
def build_no_layers_ui(parent, actions, session):
    ...
    selection_note = QtWidgets.QLabel("蒙皮图层无法附加到此对象")
    ...
    def handle_target_changed():
        ...
        note = "请选择带有 skinCluster 的网格。"
        if is_skinned:
            note = "此网格尚未初始化蒙皮图层。"
            if import_v1_actions.can_import(session):
                note = "此网格包含旧版 ngSkinTools 的蒙皮图层。"
